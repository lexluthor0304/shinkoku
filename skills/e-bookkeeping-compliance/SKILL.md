...
```
> **免責事項**: 本診断は shinkoku システムの自己評価に基づくものであり、
> 法的な適合判定を保証するものではありません。正式な適合判定が必要な場合は、
> JIIMA（公益社団法人日本文書情報マネジメント協会）の認証取得をご検討ください。
> また、優良な電子帳簿の保存の適用にあたっては、税務署への届出書の提出が必要です。
```
